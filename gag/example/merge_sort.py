...
MainSort = Sort("MainSort", [Attribute("inp_list", ListInt)], [Attribute("out_list", ListInt)])
...
Merge_S = Sort(
    "Merge_S",
    [Attribute("indices_in", Array), Attribute("res_left", Array), Attribute("res_right", Array)],
...
Split = Sort("Split", [Attribute("inp_list", ListInt), Attribute("cells", Int)], [Attribute("in_array", Array)])
MergeF = Sort("MergeF", [Attribute("cells", Int), Attribute("out_array", Array)], [Attribute("out_list", ListInt)])
DivideLeft = Sort("DivideLeft", [Attribute("inp_arr", Array)], [Attribute("arr_left", Array)])
DivideRight = Sort("DivideRight", [Attribute("inp_arr", Array)], [Attribute("arr_right", Array)])
CheckThreshold = Sort("CheckThreshold", [Attribute("inp_arr", Array)], [Attribute("state", ThresholdState)])
...
MainSortRule = Rule(
    ...
    [
        ...
        Form(RecSort, [Attribute("in_array"), Attribute("state")], [Attribute("out_array")]),
...
RecSortGeneralRule = Rule(
    Form(RecSort, [Attribute("inp_array"), Attribute("state", Literal("Rec"))], [Attribute("out_array")]),
    [
        ...
        Form(RecSort, [Attribute("arr_left"), Attribute("s1")], [Attribute("res_left")]),
        Form(RecSort, [Attribute("arr_right"), Attribute("s2")], [Attribute("res_right")]),
        
        Form(InitIndices, [], [Attribute("init_indices")]),
        Form(Merge_S, [Attribute("init_indices"), Attribute("res_left"), Attribute("res_right")], 
             [Attribute("out_array"), Attribute("indices_out")])
    ]
)

# Rule: RecSort_base (Implied by Paper description)
# Base case activated when state is "Base" (Small lists sorted locally)
RecSortBaseRule = Rule(
    Form(RecSort, [Attribute("inp_array"), Attribute("state", Literal("Base"))], [Attribute("out_array")]),
    [] # Leaf rule: actual sorting happens here
)

# --- Terminal Rules (Leafs for Local Computations) ---
GetBlocksLeaf = Rule(Form(GetBlocks, [], [Attribute("cells")]), [])
SplitLeaf = Rule(Form(Split, [Attribute("inp_list"), Attribute("cells")], [Attribute("in_array")]), [])
MergeFLeaf = Rule(Form(MergeF, [Attribute("cells"), Attribute("out_array")], [Attribute("out_list")]), [])
DivideLeftLeaf = Rule(Form(DivideLeft, [Attribute("inp_array")], [Attribute("arr_left")]), [])
DivideRightLeaf = Rule(Form(DivideRight, [Attribute("inp_array")], [Attribute("arr_right")]), [])
CheckThresholdLeaf = Rule(Form(CheckThreshold, [Attribute("inp_arr")], [Attribute("state")]), [])
InitIndicesLeaf = Rule(Form(InitIndices, [], [Attribute("init_indices")]), [])
MergeSLeaf = Rule(Form(Merge_S, [Attribute("indices_in"), Attribute("res_left"), Attribute("res_right")], 
                       [Attribute("out"), Attribute("indices_out")]), [])

# --- MergeSort GAG Instance ---
MergeSortGAG = GAG(
    sorts=[MainSort, RecSort, Merge_S, GetBlocks, Split, MergeF, DivideLeft, DivideRight, CheckThreshold, InitIndices],
