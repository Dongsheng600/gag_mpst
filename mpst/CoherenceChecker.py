...
class CoherenceChecker:
    ...
    def _explore(self, state: Dict[str, frozenset[Node]], visited: Set[Any]) -> bool:
        ...
        for p, nodes in state.items():
            for node in nodes:
                ...
                if ch is not None:
                    if is_send:
                        if ch in active_senders: return False # Linearity violation
                        active_senders[ch] = p
                    else:
                        if ch in active_receivers: return False # Linearity violation
        ...
        next_states = self._get_next_states(state, all_active)
        if not next_states:
            return False # Deadlock

        for next_s in next_states:
            if not self._explore(next_s, visited):
                return False
        
        return True

    def _get_next_states(self, state: Dict[str, frozenset[Node]], all_active: List[Tuple[str, Node]]) -> List[Dict[str, frozenset[Node]]]:
        results = []
